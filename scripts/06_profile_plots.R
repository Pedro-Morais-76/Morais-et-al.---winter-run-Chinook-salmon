...
tiff(
  file = "figures/Fig2_example_profiles.tiff",
  width = 2126,
  height = 1205,
  res = 300,
  compression = "lzw")
par(mfrow = c(2, 3), mar = c(2, 2, 2, 2), oma = c(3, 3, 1, 1))
...
for (i in seq_along(example_ids)) {
  ...
  mtext(text = expression("Otolith "^{87}*Sr*"/"^{86}*Sr),
        side = 2, line = 1, outer = TRUE, cex = 0.9)
  mtext(text = "Sr V", side = 4, line = 1, outer = TRUE, cex = 0.9)
  
  # Figure panel letter (A, B, C...)
  mtext(toupper(letters[i]), side = 3, line = 0.4, adj = 0, font = 2, cex = 0.75)
}
