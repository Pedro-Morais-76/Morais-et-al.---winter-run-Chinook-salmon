...
for (i in 1:length(FishID)){
  ...
  mtext(text="Distance from otolith core (μm)", side=1, line=0, outer=TRUE, 
        cex=1.0, font=1)
  
  mtext(text=expression(""^"87"*"Sr/"^"86"*"Sr"), side=2, line=0, outer=TRUE, 
        cex=1.0, font=1)
  
  mtext(text="Sr V", side=4, line=0, outer=TRUE, 
        cex=1.0, font=1)
}
