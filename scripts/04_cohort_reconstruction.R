...
F_plot = ggplot(subset(ages_final, Sex=="F"), aes(x=FL, fill=factor(final_age), color=factor(final_age))) +
  ...
  theme_bw() + theme(legend.position = "none"); F_plot
...
##-------------- plot all fish (males + females combined) -------------------------------------------
ALL_plot = ggplot(subset(ages_final, Sex %in% c("F", "M")), aes(x=FL, fill=factor(final_age), color=factor(final_age))) +
  ...
  labs(y="Number of fish", x = "Fork length (mm)",
       fill = "Age - years\n(scale reads)",
       color = "Age - years\n(scale reads)") +
  theme_bw() + theme(legend.position = "none"); ALL_plot


# combine plots for figure
# Extract legend from M_plot, then remove it from the plot
M_legend = cowplot::get_legend(M_plot)
M_plot_noleg = M_plot + theme(legend.position = "none")

# Left column: 3 plots stacked; right column: legend aligned to middle row
...
x = cowplot::plot_grid(plots_col, legend_col, ncol = 2, rel_widths = c(1, 0.2))

ggsave('figures/FigS1_FL_distributions.jpg', x, dpi=300, width=14, height = 19, units = "cm")
