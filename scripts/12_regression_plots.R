...
# Estimate total fraction of FW growth assimilated in any non-natal habitat and in a downstream non-natal habitat
growth_summary <- mean_fw_growth |>
  filter(Brood_year %in% yrs) |>
  group_by(Brood_year) |>
  summarize(
    tot_non_natal = sum(mean_prop_fw_growth[Habitat != "SAC"], na.rm = TRUE),
    prop_fw_growth_down = sum(mean_prop_fw_growth[!Habitat %in% c("SAC", "Unassigned", "LAS")], na.rm = TRUE),
    .groups = "drop"
  )
...
annual_df <- flow_8.1 |>
  ...
  left_join(growth_summary, by = c("BY" = "Brood_year")) |>
  left_join(juv_production, by = "BY") |>
  mutate(JPI_mill = Fry_equiv_JPI / 1000000)

# Helper function for regression p-values
format_p <- function(p) {
  if (p < 0.001) {
    "p < 0.001"
  } else {
...
pval_non_nat_text <- format_p(summary(lm_non_nat)$coefficients[2, 4])
...
rest_plot <- ggplot(annual_df, aes(x = av_flow8.1, y = tot_non_natal, color = tot)) +
  geom_smooth(method = "lm", aes(weight = tot), fill = "grey", colour = "black", linewidth = 0.6, alpha = 0.3, fullrange = TRUE) +
  geom_point(aes(size = tot), shape = 16, alpha = 0.9) + 
  geom_text_repel(
    aes(label = BY), 
    size = 3, 
...
pval_rest_text <- format_p(summary(lm_rest)$coefficients[2, 4])
...
fig4 <- cowplot::plot_grid(non_nat_plot_final, rest_plot_final, ncol = 2, rel_widths = c(1, 1.4))
ggsave(filename = "figures/Fig4_rest_stop.tif", plot = fig4, width = 15, height = 7.5, dpi = 300, units = "cm")
...
cues_plot <- ggplot(annual_df, aes(x = broodyr_day, y = prop_fw_growth_down, color = av_flow8.1)) + 
  ...
  geom_point(aes(size = tot), shape = 16, alpha = 0.65) + 
  geom_text_repel(
    aes(label = BY), 
    size = 3, 
    min.segment.length = Inf, 
...
pval_cues_text <- format_p(summary(lm_cues)$coefficients[2, 4])
...
juv_abundance_plot <- ggplot(annual_df, aes(x = JPI_mill, y = prop_fw_growth_down, color = av_flow8.1)) + 
  ...
  theme(
    ...
    axis.title.y = element_text(size = 8),
    legend.text = element_text(size = 6),
    legend.title = element_text(size = 8, lineheight = 0.8, margin = margin(b = 0))
  ) +
  guides(
...
pval_juv_text <- format_p(summary(lm_juv)$coefficients[2, 4])
...
# Combine cues and abundance plots into Figure 5
fig5 <- cowplot::plot_grid(cues_plot_final, juv_abundance_plot_final, ncol = 2, rel_widths = c(1, 1.4))

ggsave(filename = "figures/Fig5_downstream_occupancy.tif", plot = fig5, width = 15, height = 7.5, dpi = 300, units = "cm")
