...
# ------------------------------------------------------------------------------
# 4. Visualization: Annual Hydrographs (App. S1 Figure S2)
# ------------------------------------------------------------------------------

fig_width_cm <- 18
fig_height_cm <- 22 * (fig_width_cm / 25)
scale_factor <- fig_width_cm / 25
...
plot_hydrograph <- function(data_subset, label, color_vals,
  ...
  ggplot(data_subset, aes(x = broodyr_day, y = flow_cms, color = factor(BY))) +
    geom_line(linewidth = 1.1 * scale_factor) +
    theme_classic(base_size = 15 * scale_factor) +
    ylim(0, 1000) +
    labs(
      x = "Days since July 1",
...
ry07 <- plot_hydrograph(filter(df, BY %in% c(2003, 2004, 2005)), "A  |  Return year 2007", common_colors, show_y_title = TRUE,  show_x_title = FALSE)
ry08 <- plot_hydrograph(filter(df, BY %in% c(2004, 2005, 2006)), "B  |  Return year 2008", common_colors, show_y_title = FALSE, show_x_title = FALSE)

ry09 <- plot_hydrograph(filter(df, BY %in% c(2005, 2006, 2007)), "C  |  Return year 2009", common_colors, show_y_title = TRUE,  show_x_title = FALSE)
ry15 <- plot_hydrograph(filter(df, BY %in% c(2011, 2012, 2013)), "D  |  Return year 2015", common_colors, show_y_title = FALSE, show_x_title = FALSE)

ry16 <- plot_hydrograph(filter(df, BY %in% c(2012, 2013, 2014)), "E  |  Return year 2016", common_colors, show_y_title = TRUE,  show_x_title = TRUE)
ry17 <- plot_hydrograph(filter(df, BY %in% c(2013, 2014, 2015)), "F  |  Return year 2017", common_colors, show_y_title = FALSE, show_x_title = TRUE)
...
ggsave(
  'figures/AppS1_FigS2_flows_per_year.tif',
