...
sac = ggplot(filter(wr_megafile2, rearing_type=="SAC"), aes(y=Sr8786_norm, x=Distance_um, group=Sample_ID)) +
  annotate("rect", xmin = 0, xmax =1250, ymin = 0.70467, ymax = 0.7061, alpha = .3, fill="grey40")+ #Sacramento
  ...
  annotate("text", x=1230, y=.70485, label="Sacramento", size = 4, hjust = 1, angle = 0)+
  annotate("text", x=1250, y=.7094, label="Ocean",fontface="italic", size = 3.5, hjust=1) + 
...
las = ggplot(filter(wr_megafile2, rearing_type=="LAS"), aes(y=Sr8786_norm, x=Distance_um, group=Sample_ID)) +
  annotate("rect", xmin = 0, xmax =1250, ymin = 0.7037, ymax = 0.70467, alpha = .3, fill="tomato2")+ #Lassen
  geom_line(alpha = .3, size = .6)+
  labs(x="Distance (um)", y="Otolith Sr87/86")+
  geom_hline(yintercept = 0.70918, alpha = .8, linetype = "dashed")+ #Ocean
  annotate("text", x=1230, y=.7039, label="Lassen", size = 4, hjust = 1, angle = 0)+
  annotate("text", x=1250, y=.7094, label="Ocean",fontface="italic", size = 3.5, hjust=1) + 
  annotate("text", x=40, y=.710, label=paste('n =',sample_sizes_rearing_types$n_fish[sample_sizes_rearing_types$rearing_type=="LAS"]), 
           size = 4, hjust = 0)+theme_classic(base_size = 16)+
  theme(legend.position = "top",
        axis.text.x=element_blank(),
        axis.title.x=element_blank(),
...
fea = ggplot(filter(wr_megafile2, rearing_type=="DEL"), aes(y=Sr8786_norm, x=Distance_um, group=Sample_ID)) +
  annotate("rect", xmin = 0, xmax =1250, ymin = 0.7061, ymax = 0.70785, alpha = .2, fill="turquoise4")+ #FEA/Delta
  geom_line(alpha = .3, size = .6)+
  labs(x="Distance (um)", y="Otolith Sr87/86")+
  geom_hline(yintercept = 0.70918, alpha = .8, linetype = "dashed")+ #Ocean
  annotate("text", x=1230, y=.7063, label="Feather/Delta", size = 4, hjust = 1, angle = 0)+
  annotate("text", x=1250, y=.7094, label="Ocean",fontface="italic", size = 3.5, hjust=1) + 
...
ame = ggplot(filter(wr_megafile2, rearing_type=="AME"), aes(y=Sr8786_norm, x=Distance_um, group=Sample_ID)) +
  annotate("rect", xmin = 0, xmax =1250, ymin = 0.70785, ymax = 0.7102, alpha = .1, fill="turquoise")+ #American River
  geom_line(alpha = .3, size = .6)+
  labs(x="Distance (um)", y="Otolith Sr87/86")+
  geom_hline(yintercept = 0.70918, alpha = .8, linetype = "dashed")+ #Ocean
  annotate("text", x=1230, y=.7081, label="American", size = 4, hjust = 1, angle = 0)+
  annotate("text", x=1250, y=.7094, label="Ocean",fontface="italic", size = 3.5, hjust=1) + 
